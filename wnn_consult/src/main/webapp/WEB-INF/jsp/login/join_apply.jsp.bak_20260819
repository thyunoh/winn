      ...
      <div class="modal-header" style="background:#003366; padding:11px 16px;">
        <div style="display:flex; align-items:center; gap:10px; width:100%; flex-wrap:wrap;">
          ...
          <button type="button" class="btn btn-outline-light rounded px-3 py-1"
                  style="font-size:13px;" onclick="jaPdf();">
            PDF 저장 <i class="far fa-file-pdf"></i>
          </button>
...
(function(){
  ...
  /* ── PDF 저장 ────────────────────────────────────────────────────
     문서(의뢰서 + 동의서 2종)를 그대로 인쇄창에 띄운다. 브라우저의
     「대상: PDF로 저장」 으로 PDF 가 된다 — qpsChk.jsp 의 인쇄와 같은 방식이라
     별도 라이브러리가 필요 없고 한글도 깨지지 않는다. */
  window.jaPdf = function(){
    var src = gel('joinModal_in'); if (!src) return;
    var cl = src.cloneNode(true);

    // 입력칸을 값 글자로 바꾼다 — 인쇄물에 빈 네모가 찍히지 않게
    var ins = cl.querySelectorAll('input, select, textarea');
    for (var i = 0; i < ins.length; i++) {
      var el = ins[i], t = (el.type || '').toLowerCase(), rep = document.createElement('span');
      rep.className = 'pv';
      if (t === 'checkbox' || t === 'radio') rep.textContent = el.checked ? '■' : '□';
      else if (t === 'hidden' || t === 'file') { el.parentNode.removeChild(el); continue; }
      else if (t === 'password') rep.textContent = '';                    // 비밀번호는 찍지 않는다
      else if (el.tagName === 'SELECT') rep.textContent = el.selectedIndex >= 0 ? el.options[el.selectedIndex].text : '';
      else rep.textContent = el.value || '';
      el.parentNode.replaceChild(rep, el);
    }
    // 화면 전용 요소 제거 · 세 서식을 모두 펼친다
    var kill = cl.querySelectorAll('.ja-tabs, .ja-guide, .ja-warn, .ja-must, .ja-noprint, button');
    for (var k = 0; k < kill.length; k++) kill[k].parentNode.removeChild(kill[k]);
    /* ★서식마다 같은 자리에서 시작하게 한다 —
       제목을 첫 장에만 두면 2·3 쪽은 본문이 위로 붙어 시작 위치가 달라진다.
       각 장 맨 위에 같은 글꼴·같은 크기의 제목을 넣는다. */
    var PT = { f1:'컨설팅 의뢰서',
               f2:'원격접속 및 DB접근에 관한 동의서',
               f3:'개인정보 수집 및 이용에 관한 동의서' };
    var pn = cl.querySelectorAll('.ja-pane');
    for (var p = 0; p < pn.length; p++) {
      pn[p].className = 'ja-pane on';
      var key = pn[p].getAttribute('data-pane');
      if (!PT[key]) continue;
      // 카드 제목이 장 제목과 겹치는 동의서 장은 카드 제목을 뺀다
      if (key !== 'f1') {
        var h = pn[p].querySelector('.ja-card h4');
        if (h) h.parentNode.removeChild(h);
      }
      var ht = document.createElement('h1');
      ht.className = 'pdf-title';
      ht.textContent = PT[key];
      pn[p].insertBefore(ht, pn[p].firstChild);
    }
    var zb = cl.querySelector('#jaZoomBox'); if (zb) zb.style.zoom = 1;

    /* ★새 창(window.open)이 아니라 **숨은 iframe** 에 찍는다.
       팝업 차단이 켜져 있으면 새 창은 열리지 않아 아무 일도 안 일어난 것처럼 보인다.
       iframe 은 차단 대상이 아니라 어디서든 뜬다. */
    var old = gel('jaPrintFrame'); if (old) old.parentNode.removeChild(old);
    ...
    w.document.write(
      ...
      + '<title>컨설팅 의뢰서</title><style>'
      /* ★머리글(날짜)·바닥글(주소·쪽번호)은 브라우저가 찍는 것이라 CSS 로 못 지운다.
         다만 @page 여백을 0 으로 두면 그 자리가 없어져 함께 사라진다.
         대신 종이 여백은 body 안쪽 padding 으로 준다. */
      + '@page{ size:A4; margin:0; }'
      + 'body{ margin:0; padding:12mm; font-family:"맑은 고딕","Malgun Gothic",sans-serif; background:#fff; }'
      + '.pdf-title{ text-align:center; font-size:21px; font-weight:800; color:#20303a;'
      + ' letter-spacing:2px; margin:0 0 9mm; }'
      + '#joinModal_in{ background:#fff !important; padding:0 !important; }'
      + '.ja-card{ break-inside:avoid; page-break-inside:avoid; }'
      /* 2·3 쪽은 새 종이에서 시작한다. @page 여백이 0 이라 그대로 두면 제목이 종이 맨 위에 붙는다
         → body 의 위 여백(12mm)과 같은 값을 새 쪽 머리에 준다(2026-08-19 지적). */
      + '.ja-pane + .ja-pane{ page-break-before:always; padding-top:12mm; }'
      + '.pv{ font-size:12.5px; color:#20303a; }'
      + '.ja-doc{ max-height:none !important; overflow:visible !important; }'
      + '</style></head><body></body></html>');
    w.document.close();
    w.document.body.appendChild(cl);
    var go = function(){ try { w.focus(); w.print(); } catch (e) { jaMsg('error','인쇄창을 열지 못했습니다',''); } };
    var imgs = w.document.images, left = imgs.length;
    if (!left) { setTimeout(go, 300); return; }
    for (var m = 0; m < imgs.length; m++) {
      if (imgs[m].complete) { if (--left === 0) setTimeout(go, 300); }
      else imgs[m].onload = imgs[m].onerror = function(){ if (--left === 0) setTimeout(go, 300); };
    }
  };
