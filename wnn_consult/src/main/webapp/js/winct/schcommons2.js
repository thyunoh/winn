...
if (!document.getElementById('commonSearchModal')) {
	...
	$("body").append(`
	    <div class="modal fade" id="commonSearchModal" tabindex="-1" data-bs-backdrop="static" aria-hidden="true">
	        <div class="modal-dialog modal-lg modal-dialog-centered" style="max-width: 33%;">
	            <div class="modal-content shadow-lg rounded-4 border-0"
	                 style="min-height: 600px; background: white; box-shadow: 0 10px 34px rgba(0, 0, 0, 0.1);">
					...
					<!-- ✅ 초성 필터 버튼 영역 (버튼 줄여서 한 줄에 보이도록) -->
					<div id="chosungFilter" class="d-flex flex-nowrap justify-content-start px-2 py-1 border-bottom">
					    ${["ㄱ","ㄴ","ㄷ","ㄹ","ㅁ","ㅂ","ㅅ","ㅇ","ㅈ","ㅊ","ㅋ","ㅌ","ㅍ","ㅎ"]
					      .map(c => `<button class="btn btn-outline-primary btn-sm px-2 py-1 me-1" style="min-width: 34px;" 
                                 onclick="filterByChosung('${c}')">${c}</button>`).join('')}
					    <button class="btn btn-outline-primary btn-sm px-2 py-1" style="min-width: 45px;" onclick="filterByChosung('전체')">전체</button>
					</div>
