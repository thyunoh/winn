...
- **[2026-08-14 오후] 진단검사(임상병리) 판독 완주** — 39종 갈래 확정 + 어긋난 캡처 12건 재캡처.
  [판독](docs/proposals/QPS_서식판독_진단검사_2026-08-14.md) · 새 조각 [주기 복합 설계](docs/proposals/QPS_점검표엔진_주기복합_설계_2026-08-14.md).
  **다음 일감 = 「주기 복합」 구현 → 진단검사 시드**(설계상 `ROW_BLKS` 확장뿐이라 **자바 무변경 = JSP 교체로 반영**).
  ⚠***서식 이름으로 갈래를 가를 수 없다*** — 「일상/일일」로 갈리는 줄 알았으나 반증 2건이 나왔다.
  ⚠**등록 전 타 부서 대조 5건**(특히 `혈액 관리 대장 ↔ NUR011` 은 거의 같은 종이다).
