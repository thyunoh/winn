...
## 진행 중 — QPS(질향상·환자안전) 이식 · [docs/proposals/QPS_인수인계_2026-08-12.md](docs/proposals/QPS_인수인계_2026-08-12.md)

기존 프로그램(SUNWOO)의 QPS 업무를 WinCheck+ 로 옮기는 일. **가장 큰 진행 과제다.**
***"QPS 이어서" 라고만 해도 위 인수인계 문서를 먼저 읽고 착수할 것*** — 배경 재설명 불필요.
(앞 문서 [2026-08-11](docs/proposals/QPS_인수인계_2026-08-11.md) 은 **저장 엔드포인트 함정·예약 번호 표·로컬 DB 도구**용으로 계속 쓴다.)
...
- **[2026-08-12] 시설 완주** — 판독 44종 확정 · 엔진 숙제 5건 구현 · **시드 43종 적재 완료.**
  남은 것은 **간호/병동 캡처**(오면 같은 방식으로 판독→시드)와 보류 2종·재캡처 4건.
...
- ⚠**DB 는 전부 운영 적용 완료** — `docs/sql/QPS_*` 시드를 다시 돌리지 말 것(병원이 고친 서식이 되돌아간다).
