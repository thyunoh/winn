...
- **[2026-08-14] `safeRpt` 확장** — 반복행 표·서명란·정형문구를 더했다(사고·고장 보고 계열을 유형 추가로 흡수하려는 것).
  ...
  **DDL 적용 완료**(표 2개). ⚠**실동작은 아직 미검증** — 08-14 인수인계 §5.
- ★★**SUNWOO 캡처 계열은 「그 노트북」에서만 된다** — 데스크톱 조작(computer-use)이 있는 환경이 필요하다.
  이 PC 에는 MCP 서버가 하나도 없어 `SUNWOO.exe` 를 열지 못한다(2026-08-14 실측). 세션 시작 때 **환경부터 확인.**
