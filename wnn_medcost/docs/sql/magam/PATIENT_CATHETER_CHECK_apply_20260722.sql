-- =====================================================================
--  PATIENT_CATHETER_CHECK 재생성 (실행용) — 2026-07-22 : 기산 시작일 하한 = 입원일자
--
--  내용 : 2026-07-14 적용본 전체 + [추가] 첫 삽입일이 입원일자보다 이르면 입원일자로 보정
...
BEGIN
    ...
    ELSE
        ...
        ELSE
            ...
            -- ============================================================
            -- [추가 2026-07-22] 기산 시작일 하한 = 입원일자(ADMIT_DT)
            --   시작일(첫 삽입일)이 입원일자보다 이르면 입원일자로 올린다.
            --
            --   왜 필요한가 :
            --     전월 평가표가 없을 때(Select_Cnt = 0) 위 branch 가 시작일을
            --     '전월 1일' 로 잡는다. 그런데 입원일이 그보다 뒤면(예: 입원 6/19)
            --     6/1 부터 세게 되어 '입원도 안 한 기간' 이 유치일수에 들어간다.
            --     → 재원기간보다 긴 유치일수가 나오고 14일 초과가 과다 산정된다.
            --
            --   산정규칙 문서 3-1 '전월 평가표가 없으면 전월 1일부터
            --   (입원일이 전월이면 입원일부터)' 를 코드로 옮긴 것.
            --
            --   ※ 여기(첫 삽입일 확정 직후)에 한 번만 건다. 이후 루프에서
            --     cur_ins_dt 로 넘어가는 값들은 실제 기재된 재삽입일이므로 건드리지 않는다.
            -- ============================================================
...
-- =====================================================================
-- [적용 전 — 영향 대상 미리보기]
--   이번 변경으로 값이 바뀔 수 있는 후보 = 전월 평가표가 없어서 시작일이
--   '전월 1일' 로 잡히는데, 입원일이 그보다 뒤인 대상자.
--
-- SELECT LEFT(c.PAT_ID,6) AS patId, c.PAT_NM, c.ADMIT_DT, c.MED_START, c.DOC_DT
--      , CONCAT(DATE_FORMAT(DATE_SUB(CONCAT(LEFT(c.MED_START,6),'01'), INTERVAL 1 MONTH),'%Y%m'),'01') AS 종전시작일
--      , c.ADMIT_DT                                                                                    AS 변경시작일
--      , PATIENT_CATHETER_CHECK(c.HOSP_CD,c.CLFORM_VER,c.PAT_ID,c.ADMIT_DT,
...
--    AND c.EVAL_TYPE    = '2'
--    AND c.ADMIT_DT > CONCAT(DATE_FORMAT(DATE_SUB(CONCAT(LEFT(c.MED_START,6),'01'), INTERVAL 1 MONTH),'%Y%m'),'01')
--    AND NOT EXISTS ( SELECT 1 FROM TBL_PATVAL_MST p
--                      WHERE p.HOSP_CD=c.HOSP_CD AND p.CLFORM_VER=c.CLFORM_VER
--                        AND p.PAT_ID=c.PAT_ID   AND p.ADMIT_DT=c.ADMIT_DT
--                        AND p.MED_START LIKE CONCAT(DATE_FORMAT(DATE_SUB(CONCAT(LEFT(c.MED_START,6),'01'), INTERVAL 1 MONTH),'%Y%m'),'%') );
--   → 적용 전 chk 값을 저장해 두고, 적용 후 같은 쿼리로 대조한다.
--     (Y→N 으로 바뀌는 건만 있어야 한다. N→Y 가 생기면 잘못된 것)
--
-- =====================================================================
-- [적용 후 확인]
-- ① 위 미리보기 쿼리 재실행 → 종전 'Y' 중 재원기간 기준 14일 미만인 건이 'N' 으로 바뀌었는지
--
-- ② 개별 확인 (우측 그리드와 동일한 호출):
--
-- SELECT PATIENT_CATHETER_CHECK(c.HOSP_CD, c.CLFORM_VER, c.PAT_ID, c.ADMIT_DT,
--                               c.MED_START, c.EVAL_TYPE, c.INDWELL_CATH, c.PAT_CLASS) AS chk
--   FROM TBL_PATVAL_MST c
--  WHERE c.HOSP_CD = '해당병원'
--    AND c.PAT_ID LIKE '371214%'      -- 김억* : 입원 20260619 / 개시 20260701 / 작성 20260707
--    AND c.MED_START = '20260701';
--
-- ③ 종전 회귀 확인 — 주원* 461119 는 계속 'N' 이어야 한다(2026-07-14 수정분 유지)
--
-- ④ "적정성평가 월 자료생성" 재실행 → 좌측 05번 지표 분모/분자 갱신 확인
--    (좌측 지표는 자료생성 시점의 TBL_WEVALUE_MST 저장값이라 재실행해야 반영됨)
--
-- =====================================================================
-- [알려진 별건 — 이번 변경과 무관, 확인 필요]
--   위 branch 2 (라인 ~373) 의
--       SET In_Date_01 = PmDoc_Date + 1;
--   은 문자열 날짜에 숫자 1을 더하는 방식이라 월말에서 깨진다.
--       '20260630' + 1 = 20260631  (존재하지 않는 날짜)
--   같은 목적의 branch 3·4 는 DATE_ADD(...) 를 쓴다. 맞추려면:
--       SET In_Date_01 = DATE_FORMAT(DATE_ADD(STR_TO_DATE(PmDoc_Date,'%Y%m%d'), INTERVAL 1 DAY), '%Y%m%d');
--   → 전월 작성일이 말일인 대상자가 있을 때만 증상이 나온다. 별도 판단 후 반영할 것.
-- =====================================================================
