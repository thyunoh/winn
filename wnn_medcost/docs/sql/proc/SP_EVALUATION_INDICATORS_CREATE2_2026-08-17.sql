/* ======================================================================
   SP_EVALUATION_INDICATORS_CREATE2 — 수정 (2026-08-17)
   ★「적정성평가 월 자료생성」이 **실제로 쓰는** 프로시저(Magam_SQL.xml).
   ...
   ⚠원본 백업 : BACKUP_SP_EVALUATION_INDICATORS_CREATE2_20260817.sql (그대로 실행하면 원복)
   ⚠같은 로직이 SP_EVALUATION_INDICATORS_CREATE·CREATE3 에도 남아 있다 —
     지금 호출되지 않는 옛 판이라 **손대지 않았다.**
   ⚠이 프로시저는 `str_month = end_month = jobyymm` 이라 **월 루프가 한 번만** 돈다
     (값이 다음 달로 이월될 걱정이 없음을 확인함).
   ====================================================================== */
