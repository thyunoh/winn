/* ======================================================================
   SP_EVALUATION_INDICATORS_CREATE2 — 수정 (2026-09-21)
   ...
   ⚠원복 : BACKUP_SP_EVALUATION_INDICATORS_CREATE2_20260921.sql 을 그대로 실행하면 된다.
   ====================================================================== */
DROP PROCEDURE IF EXISTS SP_EVALUATION_INDICATORS_CREATE2;
DELIMITER $$
CREATE PROCEDURE SP_EVALUATION_INDICATORS_CREATE2(IN hosp_cd VARCHAR(10), IN jobyymm VARCHAR(6), IN user_id VARCHAR(50), OUT errcode VARCHAR(5), OUT errmess VARCHAR(1000))
BEGIN
...
*/
--   욕창 개선 환자분율 시작 (20260824) 변경  
		SELECT /* ★ 분모 : '제외' 혼합 케이스(악화·개선 동시 충족)를 뺀다   
		         — 그리드 select_CategoryList11 의 improveYn='2' 판정과 동일 조건 */
