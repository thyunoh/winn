...
- ~~아직 안 보는 것 : 입력 종류(CHECK/NUM/TEXT) ↔ dfm 컨트롤 종류.~~ ✅**2026-09-02 밤 봤다** — `compare.tsv` 의 dfm체크(원본 TcxCheckBox 수) ↔ 시드INPUT_GB 를
  172종 맞춰, ①시드가 전부 글자/숫자인데 원본 체크가 항목 수 이상(9종) ②시드는 체크인데 원본 체크 0(1종)을 골라 dfm 캡션·Hint 를 직접 봤다.
  진짜 「고르는 칸」은 **3종**(FAC047 구분 · NUR022 유/무 · LAB001 유/무/응급) → 입력 종류 **SEL(선택)** 신설. 결과는 [델파이소스대조 §7](../../proposals/QPS_델파이소스대조_2026-09-02.md).
  ⚠원본 체크 수만으로는 못 가른다 — 폼 전체 체크(「Check ALL」·빈 캡션 2개)가 늘 섞여 있어 **캡션·Hint 를 읽어야** 라디오 묶음인지 안다.
