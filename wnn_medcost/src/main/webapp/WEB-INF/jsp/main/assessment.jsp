    	...
    	columnsSet = [
						...
						render: function(data, type, row) {
							...
							if (type === 'display') {
								...
								/* [2026-09-03 요청] 「관리」 표시는 내보내지 않는다 — 분자 매칭 환자는
								   체크(√)와 배뇨상태로 이미 구분되어, 파란 「관리」가 한 칸 더 붙어 있으면
								   눈이 그 열에 걸린다. 「제외」는 그대로 둔다(그건 빼는 이유를 알려 주는 표시라
								   없으면 왜 분모에서 빠졌는지 알 길이 없다). */
								if (data === 'Y') {
									return '';
								}
								if (data === '제외') {
...
// [2026-07-19] 환자평가표 조회 모달 코드는 공용 /js/winmc/patvalModal.js 로 이동 (assesCheck.jsp 와 공유).
//   진입점 fn_ShowPatvalModal() 등 _pv* 전역 함수는 아래 script src 로 로드됨.
