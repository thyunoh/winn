					...
					<div class="modal-body"
						...
						<div id="inputZone">
							...
							<div class="form-group row ">
								<label for="hospCd"
									class="col-2 col-lg-2 col-form-label text-left">요양기관</label>
							...
							<div class="form-group row ">
								...
								<div class="col-4 col-lg-4">
									<select id="mainGu" name="mainGu" class="custom-select"
...
//병원병원에서 접속시 요양기관 값셋팅
	  ...
	  //원너넷이 아니거나 워너넷에서 병원을 선택해서 처리한 경우)
	    ...
	    var list_code = ['MAIN_GU']; 
	    var select_id = ['mainGu'];     // 구분코드 데이터 담길 Select (ComboBox ID)
		...
		var c_Head_Set = ['병원정보','병원명','사용아이디','사용자명','사용자구분','시작일자','종료일자','사용여부'];
		...
		var showSortNo = ['hospCd','userId','mainGuNm'];                   
		...
		function modal_key_hidden(flag) {	
	        ...
	        const inputs = [userIdInput, startDtInput,hospCdInput];
        ...
        function pass_chk(flag){
        	
		    const bfPassWd = document.getElementById('bfPassWd');
		    const afPassWd = document.getElementById('afPassWd');        	
 		    
		    if (flag == 'I') { 
               if (!bfPassWd.value.trim() || !afPassWd.value.trim()){
            	   messageBox("1","<h6>입력시 반드시 비밀번호를 입력하세요. !!</h6><p></p><br>",bfPassWd,"","");
            	   return false;
               } 
            }
            if (bfPassWd.value.trim()){
                if (bfPassWd.value != afPassWd.value){
             	   messageBox("1","<h6>비밀번호가 일치하지않습니다. !!</h6><p></p><br>",bfPassWd,"","");
             	   return false;            	   
                }            	
            } 
        	return true ;
