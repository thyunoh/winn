...
<style>
  ...
  #qpsCmplRpt .zz-tabs{ display:flex; gap:6px; margin:0 0 10px; flex-wrap:wrap; align-items:center; }
  #qpsCmplRpt .zz-tab{ border:1px solid #cfd9e0; background:#f4f7f9; color:#43555f; border-radius:8px 8px 0 0;
                   padding:7px 15px; font-size:13.5px; font-weight:700; cursor:pointer; }
  #qpsCmplRpt .zz-tab:hover{ background:#e9eff3; }
  #qpsCmplRpt .zz-tab.on{ background:#1f5a4b; border-color:#1f5a4b; color:#fff; }
  #qpsCmplRpt .zz-tab.dim{ opacity:.5; }
  #qpsCmplRpt .zz-mode{ margin-left:auto; border:1px solid #1f5a4b; background:#fff; color:#1f5a4b;
                    border-radius:8px; padding:7px 14px; font-size:13px; font-weight:700; cursor:pointer; }
  #qpsCmplRpt .zz-mode.on{ background:#1f5a4b; color:#fff; }
  #qpsCmplRpt .zz-zoom{ display:inline-flex; gap:4px; align-items:center; margin-left:2px; }
  #qpsCmplRpt .zz-zoom button{ border:1px solid #cfd9e0; background:#fff; color:#43555f; border-radius:6px;
                           padding:4px 9px; font-size:13px; font-weight:700; cursor:pointer; }
  #qpsCmplRpt .zz-zoom button:hover{ background:#eef3f6; }
</style>
...
(function(){
  ...
  (function(){
    var KEY = 'qpsTab_qpsCmplRpt', ZKEY = 'qpsZoom_qpsCmplRpt', cur = 0;
    function cards(){ return [].slice.call(document.querySelectorAll('#qpsCmplRpt .cr-card')); }
    function nm(c, i){
    ...
    function zoom(z){
      ...
      var w = document.getElementById('qpsCmplRpt');
      if (w) w.style.zoom = z.toFixed(2);
      return z;
    }
    window.zzZoom = function(d){
      var w = document.getElementById('qpsCmplRpt'), c0 = parseFloat(w && w.style.zoom) || 1;
    ...
    if (window.MutationObserver) {
      var _mw = document.getElementById('qpsCmplRpt'), _mt;
